...
2. 각 이슈에 `## 목표` / `## 배경지식` / `## 할 일` / `## 검증 방법` / `## 참고`가 있음.
   개념 설명은 이슈에 직접 안 담고 [`docs/`](docs/)로 뺐음 — 이슈끼리 개념이 중복되는 걸 막기
   위함. 이슈의 `## 배경지식` 링크를 따라가면 됨. 체크리스트를 끝내고 PR을 올리면 리뷰함.
3. PR 본문에 어떤 이슈를 다루는지 적고(`closes #N`), 커밋은 `feat:`/`fix:` 등 컨벤션대로.
4. 브랜치명은 `issue-N` 패턴.

## 배경지식 문서

개념 설명은 [`docs/`](docs/)에 페이즈별로 정리되어 있음 — 이슈 본문은 "무엇을 할지"만 담고
"왜/어떻게 이해해야 하는지"는 여기서 확인. [`docs/README.md`](docs/README.md)가 목차.

## 두 개의 트랙

- 🧠 **본 트랙 (PyTorch)** — 트랜스포머 구조를 개념 단위로 직접 구현.
- 🔧 **C 트랙** — 본 트랙에서 방금 만들고 검증한 연산을, 바로 다음 이슈에서 **C로 다시 짜서**
  `torch.utils.cpp_extension`으로 연결. 텐서 연산이 메모리에서 실제로 어떻게 도는지까지 파고듦.
  C 실력과 병행 학습. c-book 문서를 참고자료로.
...
| [#3](../../issues/3) | 🔧 | **C로 matmul 커널** + `cpp_extension` 첫 연결 (확장 빌드 파이프라인 뚫기) | #2 결과와 tolerance 내 일치 + naive/블록 버전 속도 비교 |
