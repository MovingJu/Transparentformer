...
void matmul_forward(const float* a, const float* b, float* out, long m, long k, long p) {
    ...
    std::memset(out, 0, sizeof(float) * m * p);
}
...
void sdpa_forward(const float* q, const float* k, const float* v, float* out,
    ...
    (void)q; (void)k; (void)v; (void)d_k; (void)scale;
    std::memset(out, 0, sizeof(float) * t * d_v);
}
...
void sdpa_backward(const float* q, const float* k, const float* v,
    ...
    (void)q; (void)k; (void)v; (void)grad_out; (void)d_v; (void)scale;
    std::memset(grad_q, 0, sizeof(float) * t * d_k);
    std::memset(grad_k, 0, sizeof(float) * t * d_k);
    std::memset(grad_v, 0, sizeof(float) * t * d_k);
}
...
void layernorm_forward(const float* x, const float* gamma, const float* beta, float* out,
    ...
    (void)x; (void)gamma; (void)beta; (void)eps;
    std::memset(out, 0, sizeof(float) * rows * cols);
    std::memset(mean, 0, sizeof(float) * rows);
    std::memset(rstd, 0, sizeof(float) * rows);
}
...
void layernorm_backward(const float* x, const float* gamma, const float* grad_out,
    ...
    (void)x; (void)gamma; (void)grad_out; (void)mean; (void)rstd;
    std::memset(grad_x, 0, sizeof(float) * rows * cols);
    std::memset(grad_gamma, 0, sizeof(float) * cols);
    std::memset(grad_beta, 0, sizeof(float) * cols);
}
